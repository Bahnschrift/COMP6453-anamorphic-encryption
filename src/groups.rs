...
pub trait MCG<const LIMBS: usize>: Deref<Target = ConstMontyForm<Self::MOD, LIMBS>> {
    ...
    /// The order of the group. Should be prime for secure groups.
    ...
    /// Modulus of integers the group is defined over.
    fn p() -> Uint<LIMBS>
    where
        Self: Sized,
    {
    ...
    /// Consumes an instance of the group, yielding the stored value.
    fn into_inner(self) -> ConstMontyForm<Self::MOD, LIMBS>;

    /// Tests if a given integer is a member of the group.
    ...
    /// If it is, yields an instance of `G`.
    ...
    /// Reverse operation of [`MCG::from_modq`]
    fn to_modq(&self) -> Uint<LIMBS>
    where
        Self: Sized;
}

/// Use [`crate::new_group`] to easily define new multiplicative cyclic groups.
///
/// Requires providing a group type name, modular arithmetic type name, number of limbs,
/// and initial values for `p`, `q`, and `g`.
///
/// Note that for this to behave properly and securely, you MUST ensure that
/// `g` is a generator of order `q` (in integers mod `p`), and that `q` is prime.
///
/// The existing implementations use values from <https://www.rfc-editor.org/rfc/rfc7919>.
#[macro_export]
macro_rules! new_group {
    ($name:ident, $mod_name:ident, $limbs:expr, $p:expr, $q:expr, $g:expr $(,)?) => {
        #[doc = concat!("Multiplicative cyclic group over ", stringify!($limbs), " × 64 bit integers with generator ", stringify!($g), ".")]
