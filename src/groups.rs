...
pub trait MGParams<const LIMBS: usize> {
...
/// An element of some grop G
#[derive(Debug, Clone, Copy)]
pub struct MGVal<const LIMBS: usize, G: MGParams<LIMBS>>(ConstMontyForm<G::MOD, LIMBS>);

impl<const LIMBS: usize, G: MGParams<LIMBS>> MGVal<LIMBS, G> {
    ...
    pub fn into_group(n: ConstMontyForm<G::MOD, LIMBS>) -> Option<Self> {
...
impl<const LIMBS: usize, G: MGParams<LIMBS>> std::ops::Deref for MGVal<LIMBS, G> {
    type Target = ConstMontyForm<G::MOD, LIMBS>;

    fn deref(&self) -> &Self::Target {
        let MGVal(inner) = self;
        inner
    }
}
