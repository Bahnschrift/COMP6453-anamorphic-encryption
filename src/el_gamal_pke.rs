...
impl<const LIMBS: usize, MOD: ConstMontyParams<LIMBS>> ElGamalPKE<LIMBS, MOD> {
    ...
    // Uses public key `pk` to encode message `m` given chosen random `r`
    pub fn enc_with_r(&self, pk: Uint<LIMBS>, m: Uint<LIMBS>, r: Uint<LIMBS>) -> Option<(Uint<LIMBS>, Uint<LIMBS>)> {
        if m == Uint::<LIMBS>::ZERO || m >= Self::p() {
            // Message outside of message space.
            return None;
        }
        ...
        if r >= *self.q {
            // r is not a valid exponent - must be less than subgroup order q
            return None;
        }
